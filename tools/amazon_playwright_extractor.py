...
class AmazonExtractor:
    ...
    async def _parse_keepa_ag_grid_product_details(self, keepa_frame: FrameLocator) -> Dict[str, Any]: # Removed table_container_selector, it's fixed
        ...
        try:
            ...
            # Wait for at least one row to appear in the grid with increased timeout
            try:
                await grid_root.locator("div[role='row']").first.wait_for(state="visible", timeout=7000)
            except PlaywrightTimeoutError:
                log.warning("Timeout waiting for any rows to appear in Keepa Product Details grid")
            
            # Add a small extra wait to ensure all rows are loaded
            await asyncio.sleep(1)
            
            # Now get all rows
            rows = await grid_root.locator("div[role='row']").all()
            log.info(f"Found {len(rows)} rows in Keepa Product Details AG Grid.")
            
            # Retry if no rows were found (grid might still be initializing)
            if len(rows) == 0:
                log.warning("No rows found on first attempt, waiting and retrying...")
                await asyncio.sleep(3)  # Additional wait before retry
                rows = await grid_root.locator("div[role='row']").all()
                log.info(f"Retry: Found {len(rows)} rows in Keepa Product Details AG Grid.")
            ...
            for row_loc in rows:
                ...
                if await key_cell.is_visible(timeout=1000) and await value_cell.is_visible(timeout=1000):  # Increased timeout
        ...
        try: 
            ...
            if await data_tab_button.is_visible(timeout=5000):
                ...
                if not data_tab_active: 
                    ...
                    # INITIAL WAIT for the Product-Details wrapper to appear.
                    try:
                        await keepa_frame.locator(product_details_content_selector).wait_for(
                            state="visible", timeout=11000
                        )
                    except PlaywrightTimeoutError:
                        log.error("Product-Details wrapper did not appear after 11 s – aborting.")
                        keepa_data["status"] = "Product details tab timeout (initial)"
                        return keepa_data

                    # CONDITIONAL EXTENSION ─ check if the grid container inside the tab is ready.
                    grid_container_selector = f"{product_details_content_selector} div#grid-container-product-detail"
                    if not await keepa_frame.locator(grid_container_selector).is_visible(timeout=1000):
                        log.warning(
                            "Product-Details tab visible, but inner grid still not ready – "
                            "extending wait by 5 s."
                        )
                        try:
                            await keepa_frame.locator(grid_container_selector).wait_for(
                                state="visible", timeout=5000
                            )
                            log.info("Grid became visible after extended wait – continuing.")
                        except PlaywrightTimeoutError:
                            log.error(
                                "Grid still not visible after extended 5 s – giving up on Keepa details."
                            )
