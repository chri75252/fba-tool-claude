...
**Focus on arbitrage profit margins. Clearance categories = maximum profit opportunity.**"""
    ...
    def _optimize_category_urls(self, urls: list, price_range: str = "low", max_products: Optional[int] = None) -> list:
        """Add optimization parameters to category URLs.

        ``max_products`` controls the number of products we attempt to scrape
        from each category across all its pages. This limit is inserted into the
        URL via the ``product_list_limit`` parameter so the supplier site only
        serves that many items.  It is unrelated to the price-phase logic which
        determines when we move from the £0–10 range to higher price tiers.
        """
        optimized_urls = []

        # Allow caller to pass explicit max_products. If None, load from config.
    ...
    async def _hierarchical_category_selection(self, supplier_url, supplier_name):
        ...
        if validated_urls:
            ...
            optimized_urls = self._optimize_category_urls(validated_urls, price_range=current_price_phase, max_products=max_products_per_category)
            ...
            if fallback_validated:
                optimized_urls = self._optimize_category_urls(fallback_validated, price_range="low", max_products=max_products_per_category)
...
async def run_workflow_main():
    ...
    try:
        ...
        log.info(
            "Loaded system config from %s (per_category=%s, per_cycle=%s, analyzed=%s)",
            config_path,
            max_products_per_category_cfg,
    ...
    # Expose system configuration to the workflow for limit handling
    workflow_instance.config = system_config
