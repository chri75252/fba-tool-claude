...
Return ONLY valid JSON, no additional text."""
    ...
    async def run(self, supplier_url: str = DEFAULT_SUPPLIER_URL,
                  ...
                  _ai_cycle_count: int = 0,
                  _max_ai_cycles: int = 10) -> List[Dict[str, Any]]:
        ...
        if _ai_cycle_count == 0:
            ...
            log.info(f"🔄 AI CYCLE #{_ai_cycle_count}: Max cycles allowed: {_max_ai_cycles}")

        # Check if we've exceeded maximum AI cycles
        if _ai_cycle_count >= _max_ai_cycles:
            log.warning(f"🔄 AI CYCLE LIMIT REACHED: Stopping after {_ai_cycle_count} cycles to prevent infinite loops")
        ...
        if should_continue and hasattr(self, 'ai_client') and self.ai_client and _ai_cycle_count < _max_ai_cycles:
                        ...
                        if new_products_count > 0:
                            ...
                            if max_products_to_process <= 0:
                                ...
                                additional_results = await self.run(
                                    ...
                                    _max_ai_cycles=_max_ai_cycles  # Pass through max cycles
                                )
        ...
        elif should_continue and _ai_cycle_count >= _max_ai_cycles:
            log.warning(f"🔄 NEW AI CYCLE: Would trigger new cycle but maximum cycles ({_max_ai_cycles}) reached")
